...
// GET /@scope/package/-/@scope/package-version.tgz
// GET /@scope/package/download/@scope/package-version.tgz
api.get(new RegExp('^/(@[^/]+)/([^/]+)/(-|download)/\\1/([^/]+)$'), function(req, res, match) {
  var scope = match[1],
      name = match[2],
      sep = match[3],
      file = match[4],
      uri = remoteUrl + scope + '/' + name + '/' + sep + '/' + scope + '/' + file;
  return downloadTar(req, res, uri);
});
...
// /-/ or /package/-/ or /@scope/-/ or /@scope/package/-/ are special
